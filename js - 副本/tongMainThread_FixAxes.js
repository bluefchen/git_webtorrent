...
$(function(){
    ...
    var modelDataM = [];
    var modelDataNewN = [];
    var firstDrawFileArr = [];
    var scecondDrawFileArr = [];
    var vsgData = [],vsgArr=[];
    var outsideSourcesFileCount = 0, wallFileCount = 0;
    ...
    var triggerAreaMap = [];
    var wallArr = [];
    var fileLength = 0, wallFileLength = 0;
    ...
    workerLoadVsg.onmessage=function(event) {
        ...
        wallFileLength = event.data.wallFileLength;
    ...
    function SendMessagetoWorkDforOutsideModel(datNum)
    {
        ...
        for(var i=0; i<vsgArr.length; i++) {
            ...
            if(ind=="IfcWallStandardCase" || ind=="IfcWall" || ind=="IfcBeam" || ind=="IfcSlab")
            {
                firstDrawFileArr.push(tempFileName);
            }
            else
            {
                scecondDrawFileArr.push(tempFileName);
            }
        }

        if(fileLength==0 || fileLength==null) fileLength = vsgArr.length;
        if(wallFileLength==0 || wallFileLength==null) wallFileLength = firstDrawFileArr.length;
    ...
    workerDout.onmessage = function (event) {
        ...
        if(Data.newFileName)
        {
            ...
            var pos=tempKeyValue.indexOf("=");
            var ind=tempKeyValue.substring(pos+1);
            if(ind=="IfcWallStandardCase" || ind=="IfcWall" || ind=="IfcBeam" || ind=="IfcSlab")
            {
                wallFileCount++;
            }
        ...
        console.log(outsideSourcesFileCount);
        //修改HTML标签内容
        var progress = Math.floor(100*outsideSourcesFileCount/fileLength);
        document.getElementById('progressLable').innerHTML = progress + "%";

        if(wallFileCount==wallFileLength && !isDrawWallGroup)
        {
            isDrawWallGroup = true;
            DrawModel(1);
            isOnload = false;
        }

        if(outsideSourcesFileCount==fileLength)
        {
            //修改HTML标签内容
    ...
    function DrawModel(tag)
    {
        ...
        switch (tag){
            case 1:
                fileArr = firstDrawFileArr;
                break;
            case 2:
                fileArr = scecondDrawFileArr;
                break;
            case 3:
                fileArr = vsgArr;
                break;
...
//                            处理V矩阵，变形
                if(modelDataV[newName])
                {
                    ...
                    for(var dataCount=0;dataCount<modelDataV[newName].length;dataCount++)
                    {
                        ...
                        // polyhedron.scale.set(1,1.002,1.002);
                        scene.add(polyhedron);
    ...
    function render() {
        ...
        // if(true)
        {
            ...
            if(Number(document.getElementById('isClickMap').innerHTML)==1)
            {
                ...
                camera.position.set(Number(document.getElementById('cameraPosX').innerHTML),Number(document.getElementById('cameraPosY').innerHTML),Number(document.getElementById('cameraPosZ').innerHTML));
            }
...
//                        camControls.targetObject.translateY( 5*clock.getDelta() );
    ...
    document.onkeydown=function(event){
        ...
        {
            console.log("T");
            for(var triggerMeshCounter=0; triggerMeshCounter<triggerBoxs.length; triggerMeshCounter++)
            {
                triggerBoxs[triggerMeshCounter].material.opacity = 0.5;
            }
        }
    }
